...
function cleanReport(text: string): string {
  return text
    ...
    .filter((line) => {
      const t = line.trim();
      if (!t || t === "⚠️" || t === "⚠") return false;
      if (t.includes("Disclaimer: This report was generated by AI")) return false;
      if (t.includes("This report was generated by AI")) return false;
      if (t.includes("Always verify with live sources such as CoinMarketCap")) return false;
      if (t.includes("verify with live sources such as CoinMarketCap")) return false;
      if (t.includes("Financial figures and statistics may be based on training data")) return false;
      if (t.includes("not reflect current values")) return false;
      if (t.includes("verify with CoinMarketCap, CoinGecko, Bloomberg, or Reuters")) return false;
      return true;
    })
    .join("\n")
    .trim();
